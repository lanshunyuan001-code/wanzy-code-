...
export default function ClientPage({ initialData, initialLang }: ClientPageProps) {
  ...
  useEffect(() => {
    localStorage.setItem('wanzy-lang', lang)
  }, [lang])
  ...
  return (
    ...
    >
      {/* ── Hero Header ───────────────────────────────────────── */}
      <div className="px-4 pt-8 pb-6 border-b" style={{ borderColor: '#18181b' }}>
        {/* Top bar: version badge + lang switcher */}
        ...
        {/* H1 */}
        <h1 className="text-3xl font-bold leading-tight" style={{ color: '#fafafa' }}>
          {l.HERO_TITLE}
          <br />
          <span style={{ color: '#34C759' }}>{l.HERO_SUB}</span>
        </h1>
      </div>
      ...
      >
        <div className="flex items-center gap-2">
          <div className="w-1.5 h-1.5 rounded-full animate-pulse" style={{ backgroundColor: '#34C759' }} />
          <span>{l.ONLINE}</span>
        </div>
        <span>{data.snippets.length} {l.SNIPPETS}</span>
        <span>{l.UPDATED} {data.updatedAt}</span>
      </div>
      ...
      <div className="px-4 py-6">
        ...
        {data.snippets.length > 3 && (
          <button
            ...
            style={{ color: '#3f3f46', borderColor: '#27272a', borderStyle: 'dashed' }}
            onMouseEnter={e => {
          ...
          >
            ...
            <span>{showAll ? l.SHOW_LESS : l.SHOW_MORE}</span>
          </button>
        )}
      </div>
...
function SnippetCard({ snippet, index, onClick }: SnippetCardProps) {
  ...
  return (
    <article
      ...
      style={{ backgroundColor: 'rgba(24,24,27,0.5)', borderColor: '#18181b' }}
      onMouseEnter={e => {
    ...
    >
      ...
      <div className="flex items-start justify-between gap-3 mb-2">
        <div className="flex items-center gap-3">
          <span className="text-[10px]" style={{ color: '#34C759' }}>#{num}</span>
          <span className="text-[10px]" style={{ color: '#27272a' }}>{snippet.date}</span>
        </div>
        <svg
          xmlns="http://www.w3.org/2000/svg"
          width="14"
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-1">
          <button
            ...
            style={{ color: '#27272a' }}
            onMouseEnter={e => { (e.currentTarget as HTMLElement).style.color = '#34C759' }}
            onMouseLeave={e => { (e.currentTarget as HTMLElement).style.color = '#27272a' }}
          >
